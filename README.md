...
read as *website* rather than *app* (serif headline, wide measure, no rail).
...
The test is whether removing it loses information about *what something is*, rather than
...
**Connect does not open on a listing.** The hero *is* the qualifier. The list is what you
...
"Discrete Manufacturing *or* Logistics". They are now a single `MultiSelect` with grouped
...
and the time depends on the day. The next five *weekdays* are generated client-side and
...
the whole gallery. The play glyph is a decorative `<span>` *inside* the tile's single
...
the label's initial letter, since it's assembled: *an* ERPNext, *a* Framework.
...
they enclose — space *inside* the bracket just makes it look loose, which is what a first
...
is a marker *inside* the panel: `.dialog-content:has(.fc-estimate)` in `index.css` sets the
...
the column reads equally well as *the modules this partner implements*, which makes "so
...
the project. The first invites "which ones?" and quietly redefines *everything else they do*
as *everything else inside the apps you picked* — Tridots does Helpdesk, and the project has
...
rule sits on each row's *top* edge, so "the rule below row N" is row N+1's, which is what
...
The same leading argument sets the gaps *inside* the totals. All three rows carry `mt-1.5`,
...
count — verifiable, since Frappe issues the certificates, but it says what a partner *is
qualified for*, not whether the work went well, which is the one thing this section is
about.

It's now **the share of reviewers who would recommend them**: a success signal, computed
from reviews the platform already holds, needing no validation. Over *all* of a partner's
...
render. Every partner *could* have written up a project, so "none yet" there says something
...
block with no height, border or padding collapses its margins *through* itself — which left
...
- **FNV-1a's low bits barely mix, and `% 8` reads only three of them.** Three pairs of
  partners had byte-identical review lists. Seeding each field separately did *not* fix it:
  appending a field name shifts every id's low bits by the same amount, so
  `tridots-tech` and `new-indictrans` still agreed mod 8 on the name, body *and* trade
...
The header's *label* still sits 10px right of the nav labels (46px against
...
**A hovered row hides the divider below it *and* the one above it**, so the fill reads as
...
Digital** — because they have no mark to separate out: the logo *is* the
...
Note this defers only the *display*. The inference itself is untouched: the
...
|---|---|---|
| View as | Business, Partner *(disabled)* | Wipes the store and returns to `/` |
...
- **Mock data** lives in `src/data/`. `partners.js` is the shape the list renders against
  — a real build fills it from the Partner doctype. 11 seed partners.
- **Counts don't reconcile with the list.** The map and chips show the directory's
  real figures (156 partners); the list is seeded with 13 representative ones.
  The map and the chips *do* agree with each other — both read `REGIONS`.
- **Logos are real**; the numbers beside them are not. See **Partner logos** above.
- **State is in-memory.** A refresh resets the quiz — deliberate, so the flow is easy to
  re-run while reviewing.
- **Save, Contact, Share feedback, and "Log in or create account" are inert.** They're there to
  show placement and affordance. "Book a slot" is the exception — the modal is real and
  its two-step flow works, it just doesn't submit anywhere.
- **Profile media and client logos are placeholders.** See **Partner profile** below.
...
1. **The rest of the partner profile.** The top section is built (see above); the
   About / Services and Expertise cards under it are the next pass and are
   deliberately absent rather than stubbed — an empty card reads as a bug in
   review, a missing one reads as "not yet".
2. **Contact / request flow** — what "Contact" actually opens. The button is on the
   profile and inert; it goes to the in-app messages screen, which doesn't exist.
   Not routed to a stub on purpose, so the gap stays visible.
3. **The partner-side views.** The switcher itself is built (see above) and
   `store.role` is the seam — what's missing is the screens behind it.
4. **Implementation tracking** — progress and goalposts, the collaboration half of the
   product.
