...
// $24. Partner rates are $72–145/hr, so the estimator would have quoted three
...
//
// They're now each `hours × $70`, a benchmark below every partner's published
// rate, so a pack reads as the discounted fixed-scope product it is: Tridots
// Tech at $85 estimates $3,400 bespoke against the $2,800 pack for the same 40
// hours. `hours` is the number both surfaces derive from, so they can't drift.
//
// ⚠️ $70 being under EVERY rate is load-bearing, and it's the constraint to
// check when editing the `rate` column below — a partner priced beneath the
// benchmark would have the estimator quoting less than the pack it sits next
// to, which reads as the pack being a bad deal rather than a discount.
